  ...
  <div>
    <div class="w-screen h-screen flex justify-center items-center bg-[url(/uploads/shop/banner.webp)] bg-cover bg-fixed bg-center bg-no-repeat text-center" v-if="lockedScreen || status === 'unauthenticated'">
      <div class="flex flex-col items-center">
        ...
        <input class="input input-bordered my-2" v-model="enterPW" @keydown.enter.prevent="unlockScreen()">
        <button class="btn btn-primary" @click="unlockScreen()">Enter</button>
      </div>
    </div>
    <div class="" v-else>
      ...
      <div class="flex justify-center w-full my-4 fixed z-40">
        <div class="navbar backdrop-blur-md bg-base-100/70 z-50 md:w-1/3 rounded rounded-full shadow mx-4">
      ...
      <section class="flex bg-base-300 min-h-screen">
        ...
        <div class="bg-base-200 min-h-screen w-full flex flex-col justify-between shadow shadow-l">
          <div class="" :class="(route.path.startsWith('/admin/') ? 'p-4' : 'p-0')">
            <slot/>
          </div>
          <Footer/>
        </div>
