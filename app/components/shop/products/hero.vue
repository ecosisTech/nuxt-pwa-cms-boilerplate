...
<template>
  <NuxtLink class="hero bg-no-repeat bg-center bg-center" :style="`background-image: url(${(currentProduct.images[0]) ? `/uploads/${currentProduct.images[0]}` : 'shop/product-placeholder.png'}`" :to="`/shop/product/${currentProduct.slug}`" v-if="currentProduct && currentProduct.published">
